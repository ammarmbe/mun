...
import UpdateStatus from "@/app/interviews/all/update-status";
  ...
  const columns = useMemo<ColumnDef<TInterview>[]>(
    () => [
      ...
      },
      {
        id: "date",
        accessorKey: "date",
        cell: (info) => dayjs(info.getValue() as Date).format("MMM DD, YYYY"),
  ...
  return (
    <main className="flex flex-grow flex-col">
      ...
      <div className="m-4 mt-0 flex flex-grow flex-col md:ml-3">
        ...
        <div className="mt-4 grid grid-cols-[1fr,auto,1fr] rounded-2xl border p-3 shadow-xs md:p-4">
          <button
            className={buttonStyles(
              {
                size: "sm",
          ...
          >
            {isPlaceholderData && lastClicked === "previous" ? (
              <Spinner size={20} className="self-center" />
            ) : (
